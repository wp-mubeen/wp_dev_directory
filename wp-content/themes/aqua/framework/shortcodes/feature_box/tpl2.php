<div class="<?php echo esc_attr(implode(' ', $class)); ?>" <?php if($box_bg) echo 'style="background: '.$box_bg.'"' ?>>
	<?php if(($box_image && $box_align == 'left') || ($box_image && $box_align == 'center')){ ?>
	...
	<div class="feature-block">
		<?php if($box_title){ ?>
			<h4 class="<?php echo esc_attr($cl_align); ?> <?php echo ($cl_align != 'text-center')? esc_attr('tb-border-zigzag') : esc_attr(''); ?>"><?php echo tb_filtercontent($box_title); ?></h4>
		<?php } ?>
		<?php if($box_icon){ ?>
	...
	<?php if($box_image && $box_align == 'right'){ ?>
		<div class="feature-image">
			<?php
				$attachment_image = wp_get_attachment_image_src($box_image, 'full', false);
				if($crop_image == true || $crop_image == 1):
				...
				echo '<img style="width:100%;" class="bt-image-cropped" src="'. esc_url($img_src) .'" alt="">';
			?>
			<?php if($box_colorbox_icon){ ?>
				<div class="colorbox-wrap">
					<a class="colorbox view-image" href="<?php echo esc_url($attachment_image[0]); ?>" title="<?php echo esc_attr($box_title); ?>">
